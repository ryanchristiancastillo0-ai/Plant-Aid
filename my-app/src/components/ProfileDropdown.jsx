...
function FeedbackModal({ isOpen, onClose }) {
  ...
  const SERVICE_KEY = import.meta.env.VITE_FEEDBACK_SERVICE_KEY
  const TEMPLATE_KEY = import.meta.env.VITE_FEEDBACK_TEMPLATE_KEY
  const PUBLIC_KEY = import.meta.env.VITE_FEEDBACK_PUBLIC_KEY
  ...
  return (
    ...
    >
      <div
        className="bg-white dark:bg-[#1a1a1a] w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl shadow-2xl flex flex-col max-h-[90dvh] sm:max-h-[80vh] mx-0 sm:mx-4 mb-16 sm:mb-0"
      >
        {/* drag handle */}
        ...
        <div className="flex items-center justify-between px-5 pt-3 pb-4 border-b border-zinc-100 flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-bold text-black dark:text-white">Send Feedback</h3>
          </div>
        ...
        {sent ? (
          /* success state */
          <div className="flex flex-col items-center text-center gap-3 px-5 py-8" style={{ paddingBottom: 'max(2rem, env(safe-area-inset-bottom))' }}>
            <div className="w-14 h-14 rounded-2xl bg-[#a6f2d1] flex items-center justify-center">
              <MdSend className="text-2xl text-[#1b6b51]" />
            </div>
            <p className="text-base font-bold text-black dark:text-white">Feedback sent!</p>
          ...
          <>
            ...
            <div className="overflow-y-auto flex-1 min-h-0 px-5 py-4 space-y-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  className={`px-3 py-1.5 rounded-xl text-xs font-semibold border transition-all duration-200 ${category === value ? 'bg-[#1b6b51] text-white border-[#1b6b51]' : 'bg-white dark:bg-zinc-800 text-zinc-500 dark:text-gray-400 border-zinc-200 dark:border-zinc-700 hover:border-[#1b6b51] hover:text-[#1b6b51]'}`}>
                      {label}
                    </button>
                  ))}
                </div>
            ...
            {/* fixed footer - send button always visible */}
            <div className="px-5 py-4 border-t border-zinc-100 flex-shrink-0" style={{ paddingBottom: 'max(1rem, env(safe-area-inset-bottom))' }}>
    ...
    <div className="relative" ref={dropdownRef}>
      ...
      {open && (
        <>
          ...
          <div
            className="z-50 bg-white dark:bg-[#1a1a1a] flex flex-col overflow-hidden fixed bottom-0 left-0 right-0 rounded-t-3xl shadow-2xl max-h-[85dvh] mb-16 sm:mb-0 sm:bottom-auto sm:left-auto sm:right-auto sm:top-auto sm:w-64 sm:rounded-2xl sm:border sm:border-zinc-200/80 dark:sm:border-zinc-700 sm:shadow-xl sm:shadow-zinc-900/10 dark:sm:shadow-zinc-900/20"
            style={{
          ...
          <div>
            ...
            {/* Scrollable nav */}
           {/* Scrollable nav (sign out included so it's reachable via scroll on mobile) */}
            <div
